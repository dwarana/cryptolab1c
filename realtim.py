import binascii
import itertools
import os
import sys

# C_i = P_i XOR K (where K is the reused stream cipher key)
CIPHERTEXTS_HEX = {
    #1: "0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a24d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4241d3f3585f44440ad011078381bacc075e4c3",
    2: "0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a34b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab907f18fcf347dd433bcf83432d0849e80c22b0",
    3: "",
    4: "",
    5: "",
    6: "",
    7: "",
    8: "",
    #9: "14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a902590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3",
    10: "",
    11: "",
    12: "",
    13: "0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944"
}

def clear_screen():
    if os.name == 'nt':
        os.system('cls')

def xor_bytes(b1, b2):
    return bytes([_a ^ _b for _a, _b in zip(b1, b2)])

def test_crib_at_index(xor_results, crib_text, start_index, hits_list):
    crib_bytes = crib_text.encode('ascii')
    crib_len = len(crib_bytes)
    
    if crib_len == 0:
        return

    for (i, j), p_xor_p in xor_results.items():
        if len(p_xor_p) < start_index + crib_len:
            continue

        xor_slice = p_xor_p[start_index : start_index + crib_len]

        p_result_slice_j = xor_bytes(crib_bytes, xor_slice)
        
        try:
            decoded_slice_j = p_result_slice_j.decode('ascii')

            if all(32 <= byte <= 126 for byte in p_result_slice_j):
                hits_list.append({
                    'crib_assumed_on': i,
                    'revealed_message': j,
                    'revealed_text': decoded_slice_j
                })
        except UnicodeDecodeError:
            pass

        p_result_slice_i = xor_bytes(crib_bytes, xor_slice)
        
        try:
            decoded_slice_i = p_result_slice_i.decode('ascii')
            if all(32 <= byte <= 126 for byte in p_result_slice_i):
                hits_list.append({
                    'crib_assumed_on': j,
                    'revealed_message': i,
                    'revealed_text': decoded_slice_i
                })
        except UnicodeDecodeError:
            pass

# Convert hex to bytes, truncate to shortest length
ciphertexts_bytes = {k: binascii.unhexlify(v) for k, v in CIPHERTEXTS_HEX.items()}
shortest_len = min(len(c) for c in ciphertexts_bytes.values())
ciphertexts_truncated = {k: v[:shortest_len] for k, v in ciphertexts_bytes.items()}

# Calculate P_i XOR P_j for all unique pairs
xor_pairs_p_i_p_j = {}
for i, j in itertools.combinations(ciphertexts_truncated.keys(), 2):
    xor_pair = xor_bytes(ciphertexts_truncated[i], ciphertexts_truncated[j])
    xor_pairs_p_i_p_j[(i, j)] = xor_pair

def run_realtime_crib_test():

    clear_screen()
    print("================================================================")
    print(" REAL-TIME CRIB DRAGGING ATTACK (Start Index 0 only) ")
    print("================================================================")
    print(f"Total ciphertexts: {len(CIPHERTEXTS_HEX)}. Truncated length: {shortest_len} bytes.")
    print("\nType your crib text and press ENTER. Results update instantly.")
    print("Press CTRL+C or hit Enter on an empty line to exit.")
    
    current_crib = ""
    
    while True:
        sys.stdout.write(f"\n> Crib Text: {current_crib}")
        sys.stdout.flush()
        
        try:
            new_input = input("")
            
            if new_input == "":
                break
                
            current_crib = new_input.strip()
            
        except EOFError:
            break
        except KeyboardInterrupt:
            break
        
        crib_text = current_crib

        clear_screen()
        print("================================================================")
        print(" REAL-TIME CRIB DRAGGING ATTACK (Start Index 0 only) ")
        print("================================================================")
        print(f"Total ciphertexts: {len(CIPHERTEXTS_HEX)}. Truncated length: {shortest_len} bytes.")
        print("\nType your crib text and press ENTER. Results update instantly.")
        print("Press CTRL+C or hit Enter on an empty line to exit.")
        print(f"\n> **Current Crib**: '{crib_text}' (Length: {len(crib_text)})")
        print("----------------------------------------------------------------")
        
        if not crib_text:
            print("Enter text to begin crib test...")
            continue
        
        all_hits = []
        
        START_INDEX = 0
        test_crib_at_index(xor_pairs_p_i_p_j, crib_text, start_index=START_INDEX, hits_list=all_hits)

        if not all_hits:
            print(f"No readable ASCII hits found at index {START_INDEX} for crib: '{crib_text}'")
        else:
            print(f"**Found {len(all_hits)} potential matches at index {START_INDEX}:**\n")
            
            all_hits.sort(key=lambda x: x['revealed_text'])
            
            for hit in all_hits:
                print(f" [P{hit['revealed_message']}] if P{hit['crib_assumed_on']} is crib: **{hit['revealed_text']}**")
                
    print("\n\nExiting crib dragging tool.")


if __name__ == "__main__":
    run_realtime_crib_test()